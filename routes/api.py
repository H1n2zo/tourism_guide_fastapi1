...
async def get_destination_detail(
    ...
    return {
        ...
        "average_rating": float(rating_stats.avg_rating) if rating_stats and rating_stats.avg_rating else 0
    }
...
async def get_nearby_destinations(
    ...
    # For more accuracy, use PostGIS or specialized libraries
    ...
    for dest in destinations:
        lat_diff = float(dest.latitude) - latitude
        lng_diff = float(dest.longitude) - longitude
...
async def submit_feedback(
    ...
    category: str = Query(..., regex="^(usability|features|content|design|general)$"),
    feedback: str = Query(..., min_length=10),
